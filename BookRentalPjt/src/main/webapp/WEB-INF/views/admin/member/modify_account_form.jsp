...
<link
	href="<c:url value='/resources/css/admin/modify_account_form.css' />"
			...
			<div class="modify_account_form">
				<form action="<c:url value='/admin/member/modifyAccountConfirm' />" name="modify_account_form" method="post">
					...
					<select name="a_m_gender">
						...
						<option value="M"
						<c:if test = "${gender eq 'M'}"> selected </c:if>>Man</option>
						<option value="W"
						<c:if test = "${gender eq 'W'}"> selected </c:if>>Woman</option>
					</select> <br> 
