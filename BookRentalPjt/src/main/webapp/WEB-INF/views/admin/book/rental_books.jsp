...
<link href="<c:url value='/resources/css/admin/rental_books.css' />"
				...
				<table>
					...
					<tbody>
						...
						<c:forEach var="item" items="${rentalBookVos}">
							<tr>
								<td>${item.u_m_name}<br>(${item.u_m_id})
								</td>
								<td><c:url value='/book/admin/bookDetail' var='detail_url'>
										<c:param name='b_no' value='${item.b_no}' />
									</c:url> <a href="${detail_url}">${item.b_name}</a></td>
								<td>${item.b_author}</td>
								<td>${item.b_publisher}<br>(${item.b_publish_year})
								</td>
								<td>${item.b_isbn}</td>
								<td>${item.b_call_number}</td>
								<td><c:url value='/book/admin/returnBookConfirm'
										var='return_url'>
										<c:param name='b_no' value='${item.b_no}' />
										<c:param name='rb_no' value='${item.rb_no}' />
									</c:url> <a href="${return_url}">반납</a></td>
							</tr>
						</c:forEach>
